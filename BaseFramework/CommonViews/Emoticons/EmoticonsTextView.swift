...
class EmoticonsTextView: UITextView {
    ...
    /// 插入表情符号
    func insertEmoticon(_ emoticon: Emoticons) {
        ...
        if emoticon.chs != nil {
            // 1. 创建图片附件属性文本
            let attrStr = EmoticonsAttachment.emoticonString(emoticon, height: font!.lineHeight + 4)
            
            // 2. 将文本添加到 textView
            // 1> 从 textView 拿出可变的文本
            let textStr = NSMutableAttributedString(attributedString: attributedText)
            // 2> 追加文本
            textStr.replaceCharacters(in: selectedRange, with: attrStr)
    ...
    func fullText() -> String {
        ...
        attributedText.enumerateAttributes(in: NSMakeRange(0, attrStr?.length ?? 0), options: NSAttributedString.EnumerationOptions(rawValue: 0)) { (dict, range, _) in
            ...
            if let attachment = dict["NSAttachment"] as? EmoticonsAttachment {
                ...
                let str = (attrStr!.string as NSString).substring(with: range)
                strM += str
            }
        }
